...
{ 
    ...
    m_num_Diff_Prims = 0;
    m_MaxPt = Point3f( NEG_INF, NEG_INF, NEG_INF );
    m_MinPt = Point3f( POS_INF, POS_INF, POS_INF );
    ...
    m_MinPt = Point3f( trueMin, trueMin, trueMin );
    m_MaxPt = Point3f( trueMax, trueMax, trueMax );
}
...
void UniformGrid::buildGrid(std::list<IModel*> &modelList)
{
    ...
    Vector3f delta = Vector3f(m_MinPt, m_MaxPt) * temp;
    this->m_cellSize = delta.X();

    Vector3f dReci(1.0f/delta.X(), 1.0f/delta.Y(), 1.0f/delta.Z());
    ...
    for( model = modelList.begin(); model != modelList.end(); model++ )
    {
        ...
        for ( aPrim = pPrimList.begin(); aPrim != pPrimList.end(); aPrim++ )
        {
            ...
            Point3f boxMin = primBox.getPos();
            Point3f boxMax = primBox.getPos() + primBox.getSize();
            ...
            for ( int x = x1; x <= x2; x++ )
            {
                for ( int y = y1; y <= y2; y++ )
                {
                    for ( int z = z1; z <= z2; z++ )
                    {
                        ...
                        Point3f minCorner( m_MinPt.X() + x * delta.X(), m_MinPt.Y() + y * delta.Y(), m_MinPt.Z() + z * delta.Z() );
...
bool UniformGrid::rayBoxIntersect( Ray *aRay, Point3f bmin, Point3f bmax, float* timeIn ) const
{
    Vector3f rayD = aRay->Direction();
    Point3f rayStart = aRay->Origin();

    Point3f inv_rayD(1/rayD.X(), 1/rayD.Y(), 1/rayD.Z());

    float timeOut;

    // calculate time to hit the bounding box
    Point3f tempIn((bmin.X() - rayStart.X())*inv_rayD.X(),
        (bmin.Y() - rayStart.Y())*inv_rayD.Y(),
        (bmin.Z() - rayStart.Z())*inv_rayD.Z());
    Point3f tempOut((bmax.X() - rayStart.X())*inv_rayD.X(),
        (bmax.Y() - rayStart.Y())*inv_rayD.Y(),
        (bmax.Z() - rayStart.Z())*inv_rayD.Z());

    Point3f tIn = tempIn;
    Point3f tOut = tempOut;
...
void UniformGrid::stepgrid(Ray *l_pRay, Point3f l_step, Point3f *l_tMax) const
{
    ...
    Point3f mask(tmin,tmin,tmin);
    mask.X() = (tmin == l_tMax->X())?1.0f:0.0f;
    mask.Y() = (tmin == l_tMax->Y())?1.0f:0.0f;
    mask.Z() = (tmin == l_tMax->Z())?1.0f:0.0f;

    Vector3f rayDir = l_pRay->Direction();

    Point3f tDelta( (rayDir.X()==0) ? POS_INF : abs(m_cellSize/rayDir.X()),
                    (rayDir.Y()==0) ? POS_INF : abs(m_cellSize/rayDir.Y()),
                    (rayDir.Z()==0) ? POS_INF : abs(m_cellSize/rayDir.Z()) );

    Point3f voxel = l_pRay->GetTracePosition();
	voxel = voxel + Point3f( mask.X()*l_step.X(), mask.Y()*l_step.Y(), mask.Z()*l_step.Z() );
	*l_tMax = *l_tMax + Point3f( mask.X()*tDelta.X(), mask.Y()*tDelta.Y(), mask.Z()*tDelta.Z() );
...
Point3f UniformGrid::sign(Point3f value) const
{
    Point3f ret((value.X() > 0)?1.0f:-1.0f,(value.Y() > 0)?1.0f:-1.0f, (value.Z() > 0)?1.0f:-1.0f);
    return ret;
}

Point3f UniformGrid::clamp( Point3f value, Point3f minimum, Point3f maximum ) const
{
    Point3f ret = value;
...
void UniformGrid::getVoxel(Ray *aRay) const
{
    ...
    Vector3f rayDir = aRay->Direction();
    Point3f rayPos = aRay->Origin() + (rayDir * aRay->GetTraceTime()).ToPoint();

    // clamp between 0 and m_gridDimension-1
    Point3f tfloor( floor((rayPos.X()-m_MinPt.X())*inv_len),
        floor((rayPos.Y()-m_MinPt.Y())*inv_len),
        floor((rayPos.Z()-m_MinPt.Z())*inv_len) );

    Point3f voxel = clamp( tfloor, Point3f(0.0, 0.0, 0.0), Point3f( (float)m_gridDimension-1, (float)m_gridDimension-1, (float)m_gridDimension-1) );
...
HitInfo UniformGrid::intersect(Ray& l_Ray)
{
    ...
    Point3f tMax( POS_INF, POS_INF, POS_INF );
    Pixel cell_info;
    ...
    Vector3f rayDir = l_Ray.Direction();
    Point3f rayVoxel = l_Ray.GetTracePosition();
    Point3f rayPos = l_Ray.Origin() + (rayDir * l_Ray.GetTraceTime()).ToPoint();
    ...
    Point3f outS( (float)m_gridDimension, (float)m_gridDimension, (float)m_gridDimension );
    Point3f step(1,1,1);
    Point3f pos(1,1,1);
    ...
    Point3f tempPoint = (((rayVoxel + pos) * m_cellSize + m_MinPt) - rayPos );
    ...
    {
        Point3f rayVoxelPosition = l_Ray.GetTracePosition();

        // find out if the ray has gone outside the grid
        Point3f outside( rayVoxelPosition.X() == outS.X(),
