...
{
    Point3f v1 = b2.getPos();
    Point3f v2 = b2.getPos() + b2.getSize();
    Point3f v3 = m_Position;
    Point3f v4 = m_Position + m_Size;
    return ((v4.X() > v1.X()) && (v3.X() < v2.X()) && // x-axis overlap
        (v4.Y() > v1.Y()) && (v3.Y() < v2.Y()) && // y-axis overlap
        (v4.Z() > v1.Z()) && (v3.Z() < v2.Z()));   // z-axis overlap
}

bool AABB::Contains( Point3f aPoint )
{
    Point3f v1 = m_Position, v2 = m_Position + m_Size;
...
{
    m_Position = m_Position - Point3f( delta, delta, delta );
    m_Size = m_Size + Point3f( 2*delta, 2*delta, 2*delta );
}


AABB AABB::operator+(const Point3f & p)
{
    Point3f maxp = m_Position + m_Size;
    AABB temp( Point3f( mmin(m_Position.X(), p.X()),
                        mmin(m_Position.Y(), p.Y()),
                        mmin(m_Position.Z(), p.Z()) ),
               Point3f( mmax(maxp.X(), p.X()),
                        mmax(maxp.Y(), p.Y()),
                        mmax(maxp.Z(), p.Z()) ) );
    return temp;
}
...
{
    Point3f maxp = m_Position + m_Size;
    Point3f bmaxp = b.getPos() + b.getSize();

    AABB temp( Point3f( mmin(m_Position.X(), b.getPos().X()),
                        mmin(m_Position.Y(), b.getPos().Y()),
                        mmin(m_Position.Z(), b.getPos().Z()) ),
               Point3f( mmax(maxp.X(), bmaxp.X()),
                        mmax(maxp.Y(), bmaxp.Y()),
                        mmax(maxp.Z(), bmaxp.Z()) ) );
    return temp;
}
...
AABB AABB::Intersection( AABB & b )
{
    if ( b.Intersect( *this ) )
    {
        ...
        AABB temp( Point3f( mmax(m_Position.X(), b.getPos().X()),
                            mmax(m_Position.Y(), b.getPos().Y()),
                            mmax(m_Position.Z(), b.getPos().Z()) ),
                   Point3f( mmin(maxp.X(), bmaxp.X()),
                            mmin(maxp.Y(), bmaxp.Y()),
                            mmin(maxp.Z(), bmaxp.Z()) ) );
        return temp;
    }
...
{
//    Point3f inv_rayD(1.f/r.Direction().X(), 1.f/r.Direction().Y(), 1.f/r.Direction().Z());
    float t0 = 0, t1 = INFINITY;

    Point3f tIn = (m_Position - r.Origin());//*inv_rayD;
    tIn.X() /= r.Direction().X();
    tIn.Y() /= r.Direction().Y();
    tIn.Z() /= r.Direction().Z();

    Point3f tOut = (m_Position + m_Size - r.Origin());//*inv_rayD;
